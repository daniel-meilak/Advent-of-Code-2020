...
int main(){
    ...
    for ( auto const& bag : rules ){
        ...
        while ( possible_bags.size() > 0 ){
            ...
            for ( auto contents : rules[test_bag]){
                // contents.first is the bag name in the {bag, number} pair
                possible_bags.push_back(contents.first);
            }

            // done with test bag, remove from list
            possible_bags.pop_front();
        }
    }

    std::cout << "Number of bags that can contain shiny gold bags: " << num_bags1 << std::endl;
    ...
    while ( possible_bags.size() > 0 ){
        ...
        for ( auto contents: rules[test_bag.first]){
    ...
    std::cout << "Number of bags that shiny gold bags contain: " << num_bags2 << std::endl;
